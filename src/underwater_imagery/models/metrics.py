...
def confusion_matrix(pred: Tensor, true: Tensor, num_classes: int = 8) -> Tensor:
    pred = pred.to("cpu")
    true = true.to("cpu")
...
def recall(
    pred: Tensor, true: Tensor, num_classes: int = 8, average: str = "micro"
) -> float:
    pred = pred.to("cpu")
    true = true.to("cpu")
...
def mean_pixel_acc(pred: Tensor, true: Tensor) -> t_float:
    pred = pred.to("cpu")
    true = true.to("cpu")
...
def eval(
    ...
    """Print precision, recall, mean-pixel-accuracy, f1 score, IoU Loss, Log Cross Entropy Loss, and plot confusion matrix.

    Args:
        model (nn.Module): model
        data_loader (DataLoader): dataloader
        num_classes (int, optional): number of classes. Defaults to 8.
        device (str, optional): "cpu" or "cuda" use same as model. Defaults to "cuda".
        max_iter (int, optional): Max batches to be evaluated. Defaults to 3.
    """
    precision_total = 0
    recall_total = 0
    f1_total = 0
